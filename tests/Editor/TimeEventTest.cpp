...
TEST_CASE("test_basic", "test_basic")
{
  ...
  auto event
      = *(node->emplace(node->get_time_events().begin(), time_event::exec_callback{}));
  ...
  auto event_with_expression = *(node->emplace(
      node->get_time_events().begin(), time_event::exec_callback{},
      expressions::make_expression_false()));
...
TEST_CASE("test_edition", "test_edition")
{
  ...
  auto eventA
      = *(nodeA->emplace(nodeA->get_time_events().begin(), time_event::exec_callback{}));

  auto nodeB = std::make_shared<time_sync>();
  auto eventB
      = *(nodeB->emplace(nodeB->get_time_events().begin(), time_event::exec_callback{}));

  auto nodeC = std::make_shared<time_sync>();
  auto eventC
      = *(nodeC->emplace(nodeC->get_time_events().begin(), time_event::exec_callback{}));

  auto interval1 = time_interval::create(
      ossia::time_interval::exec_callback{}, *eventA, *eventB, 1000_tv);
  auto interval2 = time_interval::create(
      ossia::time_interval::exec_callback{}, *eventB, *eventC, 1000_tv);
  auto interval3 = time_interval::create(
      ossia::time_interval::exec_callback{}, *eventA, *eventC, 2000_tv);

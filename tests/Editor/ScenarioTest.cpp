...
TEST_CASE("test_basic", "test_basic")
{
  ...
  auto start_event = *(scenar->get_start_time_sync()->emplace(
      scenar->get_start_time_sync()->get_time_events().begin(), e_callback));
  ...
  auto interval = time_interval::create(
      ossia::time_interval::exec_callback{
          [](auto&&... args) { main_interval_callback(args...); }},
      *start_event, *end_event, 1000._tv, 1000._tv, 1000._tv);
...
TEST_CASE("test_edition", "test_edition")
{
  ...
  auto start_event
      = *(start_node->emplace(start_node->get_time_events().begin(), e_callback));
...
TEST_CASE("test_execution", "test_execution")
{
  ...
  auto main_start_event = *(
      main_start_node->emplace(main_start_node->get_time_events().begin(), e_callback));
  auto main_end_event
      = *(main_end_node->emplace(main_end_node->get_time_events().begin(), e_callback));
  main_interval = time_interval::create(
      ossia::time_interval::exec_callback{
          [](auto&&... args) { main_interval_callback(args...); }},
      *main_start_event, *main_end_event, 5000._tv, 5000._tv, 5000._tv);
  ...
  auto first_start_event = *(scenario_start_node->emplace(
      scenario_start_node->get_time_events().begin(), e_callback));
  auto first_end_event = *(
      first_end_node->emplace(first_end_node->get_time_events().begin(), e_callback));
  auto first_interval = time_interval::create(
      ossia::time_interval::exec_callback{
          [=](auto&&... args) { first_interval_callback(args...); }},
      *first_start_event, *first_end_event, 1500._tv, 1500._tv, 1500._tv);
  ...
  auto second_end_event = *(
      second_end_node->emplace(second_end_node->get_time_events().begin(), e_callback));
  auto second_interval = time_interval::create(
      ossia::time_interval::exec_callback{
          [=](auto&&... args) { second_interval_callback(args...); }},
      *first_end_event, *second_end_event, 2000._tv, 2000._tv, 2000._tv);

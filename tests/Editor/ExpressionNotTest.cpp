...
{
  auto expression = make_expression_atom(false, comparator::DIFFERENT, false);
...
{
  auto exprA = make_expression_atom(true, comparator::EQUAL, true);

  auto exprB = make_expression_atom(false, comparator::EQUAL, false);

  auto exprC = make_expression_atom(true, comparator::EQUAL, true);
...
TEST_CASE("test_callback", "test_callback")
{
  ...
  auto testDestinationExpr = make_expression_atom(
      destination(*localIntAddress1), comparator::DIFFERENT,

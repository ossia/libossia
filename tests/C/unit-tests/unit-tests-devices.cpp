...
TEST_CASE("Device exceptions")
{
  ...
  SECTION("Invalid address: target node has children")
  {
    ...
    REQUIRE_THROWS(ossia::presets::apply_preset(
        localDevice, preset, ossia::presets::keep_arch_off));
  }
  ...
  SECTION("Invalid address: keep_arch_type")
  {
    ...
    REQUIRE_THROWS(
        ossia::presets::apply_preset(localDevice, preset, ossia::presets::keep_arch_on));
    REQUIRE_NOTHROW(ossia::presets::apply_preset(
        localDevice, preset, ossia::presets::keep_arch_off));
  }
}
...
TEST_CASE("Building device from preset")
{
  ...
  REQUIRE_NOTHROW(
      ossia::presets::apply_preset(localDevice, p, ossia::presets::keep_arch_off));
  ...
  SECTION("Value of nodes")
  {
    ...
    REQUIRE(
        get_node(localDevice, "/device/a.0/b.0/c.0")
            ->get_parameter()
            ->value()
            .get<int32_t>()
        == 1);
    REQUIRE(
        get_node(localDevice, "/device/a.0/b.0/c.1")
            ->get_parameter()
            ->value()
            .get<int32_t>()
        == 2);
    ...
    REQUIRE(
        get_node(localDevice, "/device/a.1/b.1/c")
            ->get_parameter()
            ->value()
            .get<int32_t>()
        == 3);
    REQUIRE(
        get_node(localDevice, "/device/a.1/b.1/d")
            ->get_parameter()
            ->value()
            .get<int32_t>()
        == 4);
...
TEST_CASE("Functions on instances")
{
  ...
  funcs.emplace_back(
      std::regex((device("device") / any_instance("a")).address),
      [&](const ossia::net::node_base& node) {
    std::cerr << "Case 1: " << ossia::net::address_string_from_node(node)
  ...
  funcs.emplace_back(
      std::regex(R"(device:/a\.?[0-9]*$)"), [&](const ossia::net::node_base& node) {
        std::cerr << "Case 3: " << ossia::net::address_string_from_node(node)
                  << " was inserted" << std::endl;
  ...
  funcs.emplace_back(
      std::regex(
          (device("device") / any_instance("a") / any_instance("b") / stop()).address),
      [&](const ossia::net::node_base& node) {
    std::cerr << "Case 2: " << ossia::net::address_string_from_node(node)
              << " was inserted" << std::endl;
  ...
  funcs.emplace_back(
      std::regex((device("device") / "a.1" / "b.1" / "d").address),
      [&](const ossia::net::node_base& node) {
    std::cerr << "Case 3: " << ossia::net::address_string_from_node(node)
              << " was inserted" << std::endl;
  ...
  REQUIRE_NOTHROW(ossia::presets::apply_preset(
      localDevice, p, ossia::presets::keep_arch_off, funcs));

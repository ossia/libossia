...
auto make_unix_client(ossia::net::network_context_ptr ctx)
{
  ...
  return ossia::net::make_osc_protocol(
      ctx, {conf::MIRROR, conf::OSC1_1, conf::COBS, conf::NEVER_BUNDLE,
            ossia::net::unix_stream_configuration{{"/tmp/ossia_cobs.server.socket"}}});
}
...
auto make_unix_server(ossia::net::network_context_ptr ctx)
{
  using conf = ossia::net::osc_protocol_configuration;
  return ossia::net::make_osc_protocol(
      ctx, {conf::HOST, conf::OSC1_1, conf::COBS, conf::NEVER_BUNDLE,
            ossia::net::unix_stream_configuration{"/tmp/ossia_cobs.server.socket"}});
}

TEST_CASE(
    "test_comm_osc_unix_cobs_server_client", "test_comm_osc_unix_cobs_server_client")
{
...
TEST_CASE(
    "test_comm_osc_unix_cobs_client_server", "test_comm_osc_unix_cobs_client_server")
{
  using namespace ossia::net;
...
TEST_CASE("test_comm_osc_unix_cobs_big", "test_comm_osc_unix_cobs_big")
{
  using namespace ossia::net;
...
TEST_CASE("test_comm_osc_unix_cobs", "test_comm_osc_unix_cobs")
{
  using namespace ossia::net;

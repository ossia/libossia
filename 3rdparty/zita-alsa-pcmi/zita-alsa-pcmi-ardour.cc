...
{
	if (_play_handle) {
    snd.pcm_close(_play_handle);
  }
  if(_capt_handle)
  {
    snd.pcm_close(_capt_handle);
  }
  if(_ctrl_handle)
  {
    snd.ctl_close(_ctrl_handle);
  }

  snd.pcm_sw_params_free(_capt_swpar);
  snd.pcm_hw_params_free(_capt_hwpar);
  snd.pcm_sw_params_free(_play_swpar);
  snd.pcm_hw_params_free(_play_hwpar);
}
...
Alsa_pcmi::pcm_start (void)
{
	...
	if (_play_handle) {
    unsigned int n = snd.pcm_avail_update(_play_handle);
    ...
    if((err = snd.pcm_start(_play_handle)) < 0)
    {
      if(_debug & DEBUG_STAT)
  ...
  if(_capt_handle && !_synced && ((err = snd.pcm_start(_capt_handle)) < 0))
  {
    if(_debug & DEBUG_STAT)
...
Alsa_pcmi::pcm_stop (void)
{
  ...
  if(_play_handle && ((err = snd.pcm_drop(_play_handle)) < 0))
  {
    if(_debug & DEBUG_STAT)
    {
  ...
  if(_capt_handle && !_synced && ((err = snd.pcm_drop(_capt_handle)) < 0))
  {
    if(_debug & DEBUG_STAT)
    {
...
snd_pcm_sframes_t Alsa_pcmi::pcm_wait(void)
{
  ...
  while(need_play || need_capt)
  {
    ...
    if(need_play)
    {
      snd.pcm_poll_descriptors(_play_handle, _poll_fd, _play_npfd);
      n1 += _play_npfd;
    }
    n2 = n1;
    if(need_capt)
    {
      snd.pcm_poll_descriptors(_capt_handle, _poll_fd + n1, _capt_npfd);
      n2 += _capt_npfd;
    }
    for(i = 0; i < n2; i++)
    ...
    timeout.tv_nsec = 0;
		...
		if (need_play) {
      snd.pcm_poll_descriptors_revents(_play_handle, _poll_fd, n1, &rev);
      if(rev & POLLERR)
      {
        if(_debug & DEBUG_WAIT)
        {
    ...
    if(need_capt)
    {
      snd.pcm_poll_descriptors_revents(_capt_handle, _poll_fd + n1, n2 - n1, &rev);
      if(rev & POLLERR)
      {
        if(_debug & DEBUG_WAIT)
        {
  ...
  play_av = 999999999;
  if(_play_handle && (play_av = snd.pcm_avail_update(_play_handle)) < 0)
  {
    _state = -1;
    if(!recover())
    {
  ...
  capt_av = 999999999;
  if(_capt_handle && (capt_av = snd.pcm_avail_update(_capt_handle)) < 0)
  {
    _state = -1;
    if(!recover())
    {
...
int Alsa_pcmi::play_init(snd_pcm_uframes_t len)
{
  ...
  if((err = snd.pcm_mmap_begin(_play_handle, &a, &_play_offs, &len)) < 0)
  {
    if(_debug & DEBUG_DATA)
      fprintf(stderr, "Alsa_pcmi: snd.pcm_mmap_begin(play): %s.\n", snd.strerror(err));
    return -1;
  }
  _play_step = (a->step) >> 3;
...
int Alsa_pcmi::capt_init(snd_pcm_uframes_t len)
{
  ...
  if((err = snd.pcm_mmap_begin(_capt_handle, &a, &_capt_offs, &len)) < 0)
  {
    if(_debug & DEBUG_DATA)
      fprintf(stderr, "Alsa_pcmi: snd.pcm_mmap_begin(capt): %s.\n", snd.strerror(err));
    return -1;
  }
  _capt_step = (a->step) >> 3;
...
Alsa_pcmi::play_done (int len)
{
  ...
  return snd.pcm_mmap_commit(_play_handle, _play_offs, len);
}
...
Alsa_pcmi::capt_done (int len)
{
  ...
  return snd.pcm_mmap_commit(_capt_handle, _capt_offs, len);
}
...
Alsa_pcmi::printinfo (void)
{
    ...
    fprintf(stdout, "  format : %s\n", snd.pcm_format_name(_play_format));
  ...
  if(_capt_handle)
  {
    ...
    fprintf(stdout, "  fsize  : %ld\n", _fsize);
    fprintf(stdout, "  nfrag  : %d\n", _capt_nfrag);
    fprintf(stdout, "  format : %s\n", snd.pcm_format_name(_capt_format));
...
Alsa_pcmi::initialise (const char* play_name, const char* capt_name, const char* ctrl_name)
{
  ...
  if(play_name)
  {
    if(snd.pcm_open(&_play_handle, play_name, SND_PCM_STREAM_PLAYBACK, 0) < 0)
    {
      _play_handle = 0;
      if(_debug & DEBUG_INIT)
      {
        fprintf(
            stderr, "Alsa_pcmi: Cannot open PCM device %s for playback.\n", play_name);
      }
    }
  }
  ...
  if(capt_name)
  {
    if(snd.pcm_open(&_capt_handle, capt_name, SND_PCM_STREAM_CAPTURE, 0) < 0)
    {
      _capt_handle = 0;
      if(_debug & DEBUG_INIT)
      {
        fprintf(
            stderr, "Alsa_pcmi: Cannot open PCM device %s for capture.\n", capt_name);
      }
    }
  }
  ...
  if(ctrl_name)
  {
    snd_alloca(&card, snd, ctl_card_info);

    if((err = snd.ctl_open(&_ctrl_handle, ctrl_name, 0)) < 0)
    {
      if(_debug & DEBUG_INIT)
      {
    ...
    if((err = snd.ctl_card_info(_ctrl_handle, card)) < 0)
    {
      if(_debug & DEBUG_INIT)
      {
  ...
  if(_capt_handle)
  {
    if(snd.pcm_hw_params_malloc(&_capt_hwpar) < 0)
    {
      if(_debug & DEBUG_INIT)
      {
    ...
    if(snd.pcm_sw_params_malloc(&_capt_swpar) < 0)
    {
      if(_debug & DEBUG_INIT)
      {
  ...
  if(_play_handle)
  {
    if(snd.pcm_hw_params_malloc(&_play_hwpar) < 0)
    {
      if(_debug & DEBUG_INIT)
      {
    ...
    if(snd.pcm_sw_params_malloc(&_play_swpar) < 0)
    {
      if(_debug & DEBUG_INIT)
      {
  ...
  if(_play_handle)
  {
    if(snd.pcm_hw_params_get_rate(_play_hwpar, &fsamp, &dir) || (fsamp != _fsamp) || dir)
    {
      if(_debug & DEBUG_INIT)
      {
    ...
    if(snd.pcm_hw_params_get_period_size(_play_hwpar, &fsize, &dir) || (fsize != _fsize)
       || dir)
    {
      if(_debug & DEBUG_INIT)
      {
    ...
    if(snd.pcm_hw_params_get_periods(_play_hwpar, &_real_nfrag, &dir)
       || (_real_nfrag != _play_nfrag) || dir)
    {
      if(_debug & DEBUG_INIT)
      {
        fprintf(
            stderr, "Alsa_pcmi warning: requested %u periods for playback, using %u.\n",
            _play_nfrag, _real_nfrag);
      }
    }

    snd.pcm_hw_params_get_format(_play_hwpar, &_play_format);
    snd.pcm_hw_params_get_access(_play_hwpar, &_play_access);
    ...
    _play_npfd = snd.pcm_poll_descriptors_count(_play_handle);
  }

  if(_capt_handle)
  {
    if(snd.pcm_hw_params_get_rate(_capt_hwpar, &fsamp, &dir) || (fsamp != _fsamp) || dir)
    {
      if(_debug & DEBUG_INIT)
      {
    ...
    if(snd.pcm_hw_params_get_period_size(_capt_hwpar, &fsize, &dir) || (fsize != _fsize)
       || dir)
    {
      if(_debug & DEBUG_INIT)
      {
    ...
    if(snd.pcm_hw_params_get_periods(_capt_hwpar, &nfrag, &dir) || (nfrag != _capt_nfrag)
       || dir)
    {
      if(_debug & DEBUG_INIT)
      {
    ...
    if(_play_handle)
    {
      _synced = !snd.pcm_link(_play_handle, _capt_handle);
    }

    snd.pcm_hw_params_get_format(_capt_hwpar, &_capt_format);
    snd.pcm_hw_params_get_access(_capt_hwpar, &_capt_access);
    ...
    _capt_npfd = snd.pcm_poll_descriptors_count(_capt_handle);
  }
...
int Alsa_pcmi::set_hwpar(
    snd_pcm_t* handle, snd_pcm_hw_params_t* hwpar, const char* sname, unsigned int nfrag,
    unsigned int* nchan)
{
	bool err;

  if(snd.pcm_hw_params_any(handle, hwpar) < 0)
  {
    if(_debug & DEBUG_INIT)
    {
  ...
  if(snd.pcm_hw_params_set_periods_integer(handle, hwpar) < 0)
  {
    if(_debug & DEBUG_INIT)
    {
  ...
  if((snd.pcm_hw_params_set_access(
          handle, hwpar,
          il ? SND_PCM_ACCESS_MMAP_INTERLEAVED : SND_PCM_ACCESS_MMAP_NONINTERLEAVED)
      < 0)
     && (snd.pcm_hw_params_set_access(
             handle, hwpar,
             il ? SND_PCM_ACCESS_MMAP_NONINTERLEAVED : SND_PCM_ACCESS_MMAP_INTERLEAVED)
         < 0)
     && (snd.pcm_hw_params_set_access(handle, hwpar, SND_PCM_ACCESS_MMAP_COMPLEX) < 0))
  {
    if(_debug & DEBUG_INIT)
    {
      fprintf(
          stderr, "Alsa_pcmi: the %s interface doesn't support mmap-based access.\n",
          sname);
    }
    return -1;
  }
  ...
  if(_debug & FORCE_16B)
  {
    err = (snd.pcm_hw_params_set_format(handle, hwpar, SND_PCM_FORMAT_S16_LE) < 0)
          && (snd.pcm_hw_params_set_format(handle, hwpar, SND_PCM_FORMAT_S16_BE) < 0);
  }
  else
  {
    err = (snd.pcm_hw_params_set_format(handle, hwpar, SND_PCM_FORMAT_FLOAT_LE) < 0)
          && (snd.pcm_hw_params_set_format(handle, hwpar, SND_PCM_FORMAT_S32_LE) < 0)
          && (snd.pcm_hw_params_set_format(handle, hwpar, SND_PCM_FORMAT_S32_BE) < 0)
          && (snd.pcm_hw_params_set_format(handle, hwpar, SND_PCM_FORMAT_S24_3LE) < 0)
          && (snd.pcm_hw_params_set_format(handle, hwpar, SND_PCM_FORMAT_S24_3BE) < 0)
          && (snd.pcm_hw_params_set_format(handle, hwpar, SND_PCM_FORMAT_S16_LE) < 0)
          && (snd.pcm_hw_params_set_format(handle, hwpar, SND_PCM_FORMAT_S16_BE) < 0);
  }
  ...
  if(err)
  {
    ...
    {
      fprintf(
          stderr, "Alsa_pcmi: no supported sample format on %s interface.\n.", sname);
    }
    return -1;
  }

  if(snd.pcm_hw_params_set_rate(handle, hwpar, _fsamp, 0) < 0)
  {
    if(_debug & DEBUG_INIT)
    {
  ...
  snd.pcm_hw_params_get_channels_max(hwpar, nchan);
  if(*nchan > 1024)
  {
    if(_debug & DEBUG_INIT)
    {
      fprintf(
          stderr, "Alsa_pcmi: detected more than 1024 %s channels, reset to 2.\n",
          sname);
    }
    *nchan = 2;
  }
  ...
  if(*nchan > MAXCHAN)
  {
    ...
    {
      fprintf(
          stderr, "Alsa_pcmi: number of %s channels reduced to %d.\n", sname, MAXCHAN);
    }
    *nchan = MAXCHAN;
  }

  if(snd.pcm_hw_params_set_channels(handle, hwpar, *nchan) < 0)
  {
    if(_debug & DEBUG_INIT)
    {
  ...
  if(snd.pcm_hw_params_set_period_size_near(handle, hwpar, &_fsize, 0) < 0)
  {
    if(_debug & DEBUG_INIT)
    {
  ...
  snd.pcm_hw_params_set_periods_min(handle, hwpar, &nf, NULL);
  if(nf < nfrag)
  {
    nf = nfrag;
  }
  if(snd.pcm_hw_params_set_periods_near(handle, hwpar, &nf, NULL) < 0)
  {
    if(_debug & DEBUG_INIT)
    {
      fprintf(
          stderr, "Alsa_pcmi: can't set %s periods to %u (requested %u).\n", sname, nf,
          nfrag);
    }
    return -5;
  }
  ...
  {
    fprintf(
        stderr, "Alsa_pcmi: use %d periods for %s (requested %u).\n", nf, sname, nfrag);
  }

  if(snd.pcm_hw_params_set_buffer_size(handle, hwpar, _fsize * nf) < 0)
  {
    if(_debug & DEBUG_INIT)
    {
      fprintf(
          stderr, "Alsa_pcmi: can't set %s buffer length to %lu.\n", sname, _fsize * nf);
    }
    return -4;
  }
  if(snd.pcm_hw_params(handle, hwpar) < 0)
  {
    if(_debug & DEBUG_INIT)
    {
...
int Alsa_pcmi::set_swpar(
    snd_pcm_t* handle, snd_pcm_sw_params_t* swpar, const char* sname)
{
	int err;

  snd.pcm_sw_params_current(handle, swpar);

  if((err = snd.pcm_sw_params_set_tstamp_mode(handle, swpar, SND_PCM_TSTAMP_MMAP)) < 0)
  {
    if(_debug & DEBUG_INIT)
    {
      fprintf(
          stderr, "Alsa_pcmi: can't set %s timestamp mode to %u.\n", sname,
          SND_PCM_TSTAMP_MMAP);
    }
    return -1;
  }
  if((err = snd.pcm_sw_params_set_avail_min(handle, swpar, _fsize)) < 0)
  {
    if(_debug & DEBUG_INIT)
    {
  ...
  if(handle == _play_handle
     && snd.pcm_sw_params_set_start_threshold(_play_handle, _play_swpar, 0U) < 0)
  {
    if(_debug & DEBUG_INIT)
    {
  ...
  if((err = snd.pcm_sw_params(handle, swpar)) < 0)
  {
    if(_debug & DEBUG_INIT)
    {
...
Alsa_pcmi::recover (void)
{
  ...
  snd_alloca(&stat, snd, pcm_status);

  if(_play_handle)
  {
    if((err = snd.pcm_status(_play_handle, stat)) < 0)
    {
      if(_debug & DEBUG_STAT)
      {
  ...
  if(_capt_handle)
  {
    if((err = snd.pcm_status(_capt_handle, stat)) < 0)
    {
      if(_debug & DEBUG_STAT)
      {
  ...
  if(_play_handle && ((err = snd.pcm_prepare(_play_handle)) < 0))
  {
    if(_debug & DEBUG_STAT)
    {
  ...
  if(_capt_handle && !_synced && ((err = snd.pcm_prepare(_capt_handle)) < 0))
  {
    if(_debug & DEBUG_INIT)
    {
...
float Alsa_pcmi::xruncheck(snd_pcm_status_t* stat)
{
  ...
  if(snd.pcm_status_get_state(stat) == SND_PCM_STATE_XRUN)
  {
    snd.pcm_status_get_tstamp(stat, &tupd);
    snd.pcm_status_get_trigger_tstamp(stat, &trig);

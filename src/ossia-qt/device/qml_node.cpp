...
void qml_node::reset_parent()
{
  ...
  if(m_ossia_node)
  {
    ...
    if(par)
    {
      ...
      if(!reading)
      {
        // qDebug() << ossia::net::address_string_from_node(*par).c_str() << " had " <<
        // par->children().size() << " children";

        par->remove_child(*node);
        // qDebug() << ossia::net::address_string_from_node(*par).c_str() << " now has "
        // << par->children().size() << " children";
      }
    }
  }
...
void qml_node::resetNode()
{
  ...
  if(m_device)
  {
    ...
    auto setup_valid_node = [&] {
      m_ossia_node->about_to_be_deleted.connect<&qml_node::on_node_deleted>(this);

...
public:
  void intervalChanged(qml_interval* interval)
      E_SIGNAL(OSSIA_EXPORT, intervalChanged, interval);
  void startTrigChanged(QQmlScriptString arg_1)
      E_SIGNAL(OSSIA_EXPORT, startTrigChanged, arg_1);
  void endTrigChanged(QQmlScriptString arg_1)
      E_SIGNAL(OSSIA_EXPORT, endTrigChanged, arg_1);
  void startCondChanged(QQmlScriptString arg_1)
      E_SIGNAL(OSSIA_EXPORT, startCondChanged, arg_1);
  void endCondChanged(QQmlScriptString arg_1)
      E_SIGNAL(OSSIA_EXPORT, endCondChanged, arg_1);
...
private:
  ...
  W_PROPERTY(
      QQmlScriptString, endCond READ endCond WRITE setStartCond NOTIFY endCondChanged)

  W_PROPERTY(
      QQmlScriptString,
      startCond READ startCond WRITE setStartCond NOTIFY startCondChanged)

  W_PROPERTY(
      QQmlScriptString, endTrig READ endTrig WRITE setStartTrig NOTIFY endTrigChanged)

  W_PROPERTY(
      QQmlScriptString,
      startTrig READ startTrig WRITE setStartTrig NOTIFY startTrigChanged)

  W_PROPERTY(
      qml_interval*, interval READ interval WRITE setInterval NOTIFY intervalChanged)
};
}
}

...
struct oscquery_protocol_common
{
  ...
  static void osc_send_message(Protocol& proto, Socket& socket, const Addr& addr, const ossia::value& val)
  {
    ...
    using send_visitor = osc_value_send_visitor<Addr, OscVersion, udp_socket>;
    send_visitor vis{addr, ossia::net::osc_address(addr), socket};
    val.apply(vis);
  }
  template<typename Protocol, typename Addr>
  static void osc_send_message(Protocol& proto, const Addr& addr, const ossia::value& val)
  {
    osc_send_message(proto, proto.osc_sender(), addr, val);
  }

  template<typename Protocol, typename Socket, typename Addr>
  static void ws_send_binary_message(Protocol& proto, Socket&& socket, const Addr& addr, const ossia::value& val)
  {
    using namespace ossia::net;
  ...
  template<typename Protocol, typename Addr>
  static void ws_send_binary_message(Protocol& proto, const Addr& addr, const ossia::value& val)
  {
    using namespace ossia::net;
    using write_visitor = osc_value_write_visitor<Addr, OscVersion>;
...
struct oscquery_protocol_client : oscquery_protocol_common<OscVersion>
{
  ...
  static bool push(T& self, const Addr_T& addr, Val_T&& v)
  {
    ...
    return oscquery_protocol_common<OscVersion>::push(self, addr, std::forward<Val_T>(v));
  }
  ...
  static bool push_bundle(Protocol& proto, const Addresses& addresses)
  {
    if(auto bundle = ossia::net::make_bundle(ossia::net::bundle_client_policy<OscVersion>{}, addresses))
    {
      ...
      {
        proto.ws_client().send_binary_message({bundle->data.data(), bundle->data.size()});
      }
      return true;
    }
    return false;

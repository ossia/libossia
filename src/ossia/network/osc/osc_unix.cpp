...
bool osc_unix_protocol::push(const ossia::net::parameter_base& addr, const ossia::value& v)
{
  ...
  if (val.valid())
  {
    val.apply(osc_value_send_visitor<unix_socket>{addr.get_node().osc_address(), m_impl->to_client});
    return true;
  }
  return false;
}
...
bool osc_unix_protocol::push(const ossia::net::parameter_base& addr, ossia::value&& v)
{
  ...
  auto val = filter_value(addr, std::move(v));
  if (val.valid())
  {
    val.apply(osc_value_send_visitor<unix_socket>{addr.get_node().osc_address(), m_impl->to_client});
    return true;
...
bool osc_unix_protocol::push_raw(const ossia::net::full_parameter_data& addr)
{
  ...
  if (val.valid())
  {
    val.apply(osc_value_send_visitor<unix_socket>{addr.address, m_impl->to_client});
    return true;
  }
  return false;
}

...
public:
  ...
  using encoder = typename Framing::template encoder<multi_socket_writer<std::unique_ptr<listener>>>;
  using decoder = typename Framing::template decoder<socket>;
...
struct tcp_size_prefix_server
    : framed_server<tcp_server, size_prefix_framing> {
  using framed_server::framed_server;
};
struct tcp_slip_server
    : framed_server<tcp_server, slip_framing> {
  using framed_server::framed_server;
};

struct tcp_size_prefix_client
    : framed_client<tcp_client, size_prefix_framing> {
  using framed_client::framed_client;
};
struct tcp_slip_client
    : framed_client<tcp_client, slip_framing> {
  using framed_client::framed_client;
};
...
struct unix_stream_slip_server
    : framed_server<unix_stream_server, slip_framing> {
  using framed_server::framed_server;
};
...
struct unix_stream_slip_client
    : framed_socket<unix_stream_client, slip_framing> {
  using framed_socket::framed_socket;
};
#endif

...
/*
   ...
   Repackaged as a standalone header for ossia.
*/
...
inline std::string base64_decode(std::string const& input)
{
  ...
  auto is_base64 = [](unsigned char c) -> bool {
    return (c == 43 || (c >= 47 && c <= 57) || (c >= 65 && c <= 90)
            || (c >= 97 && c <= 122));
  };
  ...
  if(i)
  {
    ...
    {
      const char* p = base64_chars;
      while(*p && *p != char_array_4[j])
        ++p;
      char_array_4[j] = static_cast<unsigned char>(p - base64_chars);
    }

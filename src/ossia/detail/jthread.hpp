...
// Polyfill until libc++ gets jthread.
// License: CC-4.0
// https://github.com/StirlingLabs/jthread/blob/main/LICENSE
...
public:
  ...
  bool __try_add_callback(
      __stop_callback_base* __cb, bool __incrementRefCountIfSuccessful) noexcept
  {
...
private:
  ...
  static constexpr std::uint64_t __source_ref_increment = static_cast<std::uint64_t>(1u)
                                                          << 33u;
...
public:
  ...
  operator==(const stop_token& __a, const stop_token& __b) noexcept
  {
...
public:
  ...
  template <
      typename _CB, std::enable_if_t<std::is_constructible_v<_Callback, _CB>, int> = 0>
...
public:
  ...
  template <
      ...
      typename
      = ::std::enable_if_t<!::std::is_same_v<::std::decay_t<Callable>, jthread>>>
...
inline jthread::jthread(Callable&& cb, Args&&... args)
  ...
  if constexpr(std::is_invocable_v<Callable, stop_token, Args...>)
  {
    ...
    ::std::invoke(
        ::std::forward<decltype(cb)>(cb), std::move(st),

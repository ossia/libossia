 ...
 *                                                                             *
 * The following constants should only be changed with caution because * they
 *have been aligned to maintain internal consistency. The default values *
...
typedef double tsReal;
   ...
   *                                                                             *
   *     tsStatus status; * TS_TRY(any_label, status.code, &status) *
   *         // Wrap function calls with TS_CALL. * TS_CALL(any_label,
   *status.code, ts_bspline_to_beziers(              * &spline, &beziers,
   *&status))                                    * if (...) *
   *             // Use one of the TS_THROW macros to raise an error. *
   *             TS_THROW_0(any_label, &status, TS_MALLOC, "out of memory") *
   *     TS_CATCH(status.code) *
   *         // Executed on error. * printf(status.message); * TS_FINALLY *
   *         // Executed in any case. * TS_END_TRY *
   *                                                                             *
...
#define TS_THROW_3(label, error, status, val, msg, arg1, arg2, arg3) \
   ...
   * lines, and points. NURBS use homogeneous coordinates to store their
   * control points (i.e. the last component of a control point stores the
   * weight). Bezier curves are B-Splines with 'num_control_points == order'
   * and a clamped knot vector, which lets them pass through their first and
   * last control point (a property which does not necessarily apply to
   * B-Splines and NURBS). Lines and points, on that basis, are Bezier curves
   * of degree 1 (lines) and 0 (points).
   *
   ...
   * spline at given knots by iteratively computing a net of intermediate
   * values until the result is available:
   *
   ...
   * (cf. tsBSpline).
   *
   * There is a special case in which the evaluation of a knot value 'u'
   * returns two results instead of one. It occurs when the multiplicity of 'u'
   * ( s(u) ) is equals to the order of a spline, indicating that the
   * corresponding spline is discontinuous at 'u'. This is common practice for
   * B-Splines (and NURBS) consisting of connected Bezier curves where the
   * endpoint of curve 'c_i' is equal to the start point of curve 'c_i+1'. Yet,
   * the end point of 'c_i' and the start point of 'c_i+1' may still be
   * completely different, yielding to visible gaps. In such case (s(u) ==
   * order), 'points' stores only the two resultant points (there is no net to
   * calculate) and 'result' points to the *first* point in 'points'. Since
   * having gaps in splines is unusual, both points in 'points' are generally
   * equals, making it easy to handle this special case by accessing 'result'
   ...
   *                                           // the second result.
   *
   * As if this wasn't complicated enough, there is an exception for this
   * special case, yielding to exactly one result (just like the regular case)
   * even if 's(u) == order'. It occurs when 'u' is the lower or upper bound of
   * a spline's domain. For instance, if 'b' is a spline with domain [0, 1] and
   * is evaluated at 'u = 0' or 'u = 1' then 'result' is *always* a single
   * point regardless of the multiplicity of 'u'.
   *
   * In summary, three different types of evaluation exist: i) the regular
   * case, in which all points of the corresponding net are returned, ii) a
   * special case, in which exactly two points are returned (required for
   * splines with gaps), and iii) the exception of ii), in which exactly one
   * point is returned even if 's(u) == order'. All in all this looks quite
   * complex (and actually it is), but for most applications you do not have to
   * deal with this. Just use 'ts_deboornet_result' to access the outcome of De
   * Boor's algorithm.
   */
  typedef struct
  {
  ...
  ts_bspline_set_degree(tsBSpline* spline, size_t deg, tsStatus* status);
   ...
   */
  tsError TINYSPLINE_API
  ts_bspline_set_order(tsBSpline* spline, size_t order, tsStatus* status);

  /**
   * Returns the dimension of \p spline. The dimension of a spline describes
   * the number of components for each point in
   * ts_bspline_get_control_points(spline). One-dimensional splines are
  ...
  /**
   * Sets the dimension of \p spline. The following conditions must be
   * satisfied:
   *
   * 	(1) dim >= 1
   * 	(2) len_control_points(spline) % dim == 0
   *
   * with _len_control_points_ being the length of the control point array of
   * \p spline. The dimension of a spline describes the number of components
   * for each point in ts_bspline_get_control_points(spline). One-dimensional
   * splines are possible, albeit their benefit might be questionable.
   *
   * @param[out] spline
   ...
   */
  tsError TINYSPLINE_API
  ts_bspline_set_dimension(tsBSpline* spline, size_t dim, tsStatus* status);
  ...
  /**
   * Returns the size of the control point array of \p spline. This function
   * may be useful when copying control points using memcpy or memmove.
   *
   * @param[in] spline
   ...
   */
  tsError TINYSPLINE_API ts_bspline_control_points(
      const tsBSpline* spline, tsReal** ctrlp, tsStatus* status);
   ...
   * 	If allocating memory failed.
   */
  tsError TINYSPLINE_API ts_bspline_control_point_at(
      const tsBSpline* spline, size_t index, tsReal** ctrlp, tsStatus* status);
   ...
   * 	On success.
   */
  tsError TINYSPLINE_API ts_bspline_set_control_points(
      tsBSpline* spline, const tsReal* ctrlp, tsStatus* status);
   ...
   * 	If \p index is out of range.
   */
  tsError TINYSPLINE_API ts_bspline_set_control_point_at(
      tsBSpline* spline, size_t index, const tsReal* ctrlp, tsStatus* status);
  ...
  /**
   * Returns the size of the knot array of \p spline. This function may be
   * useful when copying knots using memcpy or memmove.
   *
   * @param[in] spline
   ...
   */
  tsError TINYSPLINE_API
  ts_bspline_knots(const tsBSpline* spline, tsReal** knots, tsStatus* status);
   ...
   * 	If \p index is out of range.
   */
  tsError TINYSPLINE_API ts_bspline_knot_at(
      const tsBSpline* spline, size_t index, tsReal* knot, tsStatus* status);
   ...
   */
  tsError TINYSPLINE_API ts_bspline_set_knots(
      tsBSpline* spline, const tsReal* knots, tsStatus* status);
   ...
   */
  tsError TINYSPLINE_API ts_bspline_set_knot_at(
      tsBSpline* spline, size_t index, tsReal knot, tsStatus* status);
  ...
  /**
   * Returns the number of insertion that were necessary to evaluate the knot
   * of \p net.
   *
   * @param[in] net
  ...
  /**
   * Returns the dimension of \p net. The dimension of a net describes the
   * number of components for each point in ts_bspline_get_points(spline).
   ...
   * 	If allocating memory failed.
   */
  tsError TINYSPLINE_API ts_deboornet_points(
      const tsDeBoorNet* net, tsReal** points, tsStatus* status);
  ...
  /**
   * Returns the size of the result array of \p net. This function may be
   * useful when copying results using memcpy or memmove.
   *
   * @param[in] net
   ...
   * 	If allocating memory failed.
   */
  tsError TINYSPLINE_API ts_deboornet_result(
      const tsDeBoorNet* net, tsReal** result, tsStatus* status);
   ...
   * 	If allocating memory failed.
   */
  tsError TINYSPLINE_API ts_bspline_new(
      size_t num_control_points, size_t dimension, size_t degree,
      tsBSplineType type, tsBSpline* spline, tsStatus* status);

  /**
   * Creates a deep copy of \p src and stores the copied values in \p dest.
   * Does nothing, if \p src == \p dest.
   *
   * @param[in] src
   ...
   */
  tsError TINYSPLINE_API
  ts_bspline_copy(const tsBSpline* src, tsBSpline* dest, tsStatus* status);
   ...
   * 	If allocating memory failed.
   */
  tsError TINYSPLINE_API ts_deboornet_copy(
      const tsDeBoorNet* src, tsDeBoorNet* dest, tsStatus* status);
   ...
   *
   * Note: \p num_points is the number of points in \p points and not the
   * length of \p points. For instance, the following point vector has \p
   * num_points = 4 and \p dimension = 2:
   *
   ...
   * control points into a sequence of bezier curves. In order to avoid
   * division by zero, successive control points with distance less than or
   * equal to \p epsilon are filtered out. If the resultant sequence contains
   * only a single point, a spline of degree 0 (a point) is created.
   * Optionally, the first and last control point can be specified (\p first
   * and \p last).
   *
   * @param[in] points
   * 	The points to interpolate.
   ...
   * 	If allocating memory failed.
   */
  tsError TINYSPLINE_API ts_bspline_interpolate_catmull_rom(
      const tsReal* points, size_t num_points, size_t dimension, tsReal alpha,
      const tsReal* first, const tsReal* last, tsReal epsilon,
      tsBSpline* spline, tsStatus* status);
   ...
   ******************************************************************************/
  /**
   * Evaluates \p spline at knot \p u and stores the result (cf. tsDeBoorNet)
   * in \p net.
   *
   * @param[in] spline
   ...
   * 	If allocating memory failed.
   */
  tsError TINYSPLINE_API ts_bspline_eval(
   ...
   * created and reused for all evaluation tasks (therefore the memory
   * footprint is reduced to a minimum).
   *
   * @param[in] spline
   * 	The spline to evaluate.
   * @param[in] us
   ...
   * 	If allocating memory failed.
   */
  tsError TINYSPLINE_API ts_bspline_eval_all(
      const tsBSpline* spline, const tsReal* us, size_t num, tsReal** points,
      tsStatus* status);

  /**
   * Generates a sequence of \p num different knots (The knots are equally
   * distributed between the minimum and the maximum of the domain of \p
   * spline), passes this sequence to ts_bspline_eval_all, and stores the
   * resultant points in \p points. If \p num is 0, the following default is
   * taken as fallback:
   *
   ...
   * of stability regarding future changes, the actual number of generated
   * knots (which only differs from \p num if \p num is 0) is stored in \p
   * actual_num. If \p num is 1, the point located at the minimum of the domain
   * of \p spline is evaluated.
   *
   * @param[in] spline
   * 	The spline to evaluate.
   * @param[in] num
   ...
   * 	If allocating memory failed.
   */
  tsError TINYSPLINE_API ts_bspline_sample(
      const tsBSpline* spline, size_t num, tsReal** points, size_t* actual_num,
      tsStatus* status);
   ...
   *
   * This function is using the bisection method to determine P. Accordingly,
   * it is expected that the control points of \p spline are sorted at
   * component \p index either in ascending order (if \p ascending != 0) or in
   * descending order (if \p ascending == 0). If the control points of \p
   * spline are not sorted at component \p index, the behaviour of this
   * function is undefined. For the sake of fail-safeness, the distance of
   * P[index] and \p value is compared with the absolute value of \p epsilon
   * (using fabs).
   *
   * The bisection method is an iterative approach which minimizes the error
   * (\p epsilon) with each iteration step until an "optimum" was found.
   * However, there may be no point P satisfying the distance condition. Thus,
   * the number of iterations must be limited (\p max_iter). Depending on the
   * domain of the control points of \p spline at component \p index and \p
   * epsilon, \p max_iter ranges from 7 to 50. In most cases \p max_iter == 30
   * should be fine though. The parameter \p persnickety allows to define the
   * behaviour of this function is case no point was found after \p max_iter
   * iterations. If enabled (!= 0), TS_NO_RESULT is returned. If disabled (==
   * 0), the best fitting point is returned.
   *
   * @param[in] spline
   * 	The spline to evaluate
   * @param[in] value
   ...
   * 	If allocating memory failed.
   */
  tsError TINYSPLINE_API ts_bspline_bisect(
      const tsBSpline* spline, tsReal value, tsReal epsilon, int persnickety,
      size_t index, int ascending, size_t max_iter, tsDeBoorNet* net,
      tsStatus* status);
   ...
   */
  void TINYSPLINE_API
  ts_bspline_domain(const tsBSpline* spline, tsReal* min, tsReal* max);
   ...
   * 	If allocating memory failed.
   */
  tsError TINYSPLINE_API ts_bspline_is_closed(
      const tsBSpline* spline, tsReal epsilon, int* closed, tsStatus* status);
   ...
   * with the other parameters required to calculate the actual
   *transformation). * By passing a different pointer to the output parameter,
   *the transformation  * result is calculated and stored without changing the
   *state of the input.    * This is in particular useful when dealing with
   *errors as the original state * will never be modified. For instance, let's
   *have a look at the following    * code snippet: *
   *                                                                             *
   *     tsBSpline in = ...    // an arbitrary spline * tsBSpline out;  //
   *result of transformation                       *
   *                                                                             *
   *     // Subdivide 'in' into sequence of bezier curves and store the result
   **
   *     // in 'out'. Does not change 'in' in any way. * tsError err =
   *ts_bspline_to_beziers(&in, &out);                         * if (err !=
   *TS_SUCCESS) {                                                *
   ...
   **
   *                                                                             *
   * Note: None of TinySpline's transformation functions frees the memory of
   *the * output parameter. Thus, when using the same output parameter multiple
   ** times, make sure to free memory before each call. Otherwise, you will *
   *       have a bad time with memory leaks: *
   *                                                                             *
   *     tsBSpline in = ...                  // an arbitrary spline * tsBSpline
   *out;                      // result of transformations        *
   *                                                                             *
   ...
   *output, * pass it as input and output at once: *
   *                                                                             *
   ...
   * rather than _p+1_. The derivative of a point (degree == 0) is another
   *point with coordinate 0.
   *
   * @param[in] spline
   ...
   * 	If allocating memory failed.
   */
  tsError TINYSPLINE_API ts_bspline_derive(
      const tsBSpline* spline, size_t n, tsReal epsilon, tsBSpline* derivative,
      tsStatus* status);
   ...
   * 	If allocating memory failed.
   */
  tsError TINYSPLINE_API ts_bspline_insert_knot(
      const tsBSpline* spline, tsReal u, size_t num, tsBSpline* result,
      size_t* k, tsStatus* status);

  /**
   * Splits \p spline at knot value \p u and stores the result in \p split.
   * That is, \p u is inserted _n_ times such that the multiplicity of \p u is
   * equal to the \p spline's order. Creates a deep copy of \p spline if \p
   * spline != \p split.
   *
   * @param[in] spline
   * 	The spline to split.
   ...
   * 	If allocating memory failed.
   */
  tsError TINYSPLINE_API ts_bspline_split(
      const tsBSpline* spline, tsReal u, tsBSpline* split, size_t* k,
      tsStatus* status);
   ...
   * last control point; 1 => keeps the original shape). If \p tension < 0 or
   * if \p tension > 1, the behaviour of this function is undefined, though, it
   * will not result in an error. Creates a deep copy of \p spline if \p spline
   * != \p out.
   *
   ...
   * 	If allocating memory failed.
   */
  tsError TINYSPLINE_API ts_bspline_tension(
      const tsBSpline* spline, tsReal tension, tsBSpline* out,
      tsStatus* status);
   ...
   * 	If allocating memory failed.
   */
  tsError TINYSPLINE_API ts_bspline_to_beziers(
      const tsBSpline* spline, tsBSpline* beziers, tsStatus* status);
   ...
   ******************************************************************************/
  /**
   * Serializes \p spline to a null-terminated JSON string and stores the
   * result in \p json.
   *
   * @param[in] spline
   ...
   */
  tsError TINYSPLINE_API
  ts_bspline_to_json(const tsBSpline* spline, char** json, tsStatus* status);
   ...
   */
  tsError TINYSPLINE_API
  ts_bspline_from_json(const char* json, tsBSpline* spline, tsStatus* status);
   ...
   */
  tsError TINYSPLINE_API
  ts_bspline_save(const tsBSpline* spline, const char* path, tsStatus* status);
   ...
   */
  tsError TINYSPLINE_API
  ts_bspline_load(const char* path, tsBSpline* spline, tsStatus* status);
  ...
  ts_distance(const tsReal* x, const tsReal* y, size_t dimension);
#ifdef __cplusplus
}
#endif
...
tsError ts_int_deboornet_new(
    const tsBSpline* spline, tsDeBoorNet* net, tsStatus* status);

...
{
  return sizeof(struct tsBSplineImpl) + ts_bspline_sof_control_points(spline)
    ...
    const tsBSpline* spline, size_t index, tsReal** ctrlp, tsStatus* status)
{
  ...
  {
    TS_RETURN_2(
        status, TS_INDEX_ERROR, "index (%lu) >= num(control_points) (%lu)",
        (unsigned long)index,
    ...
    const tsBSpline* spline, size_t index, tsReal* knot, tsStatus* status)
{
  ...
  {
    TS_RETURN_2(
        status, TS_INDEX_ERROR, "index (%lu) >= num(knots) (%lu)",
        (unsigned long)index, (unsigned long)ts_bspline_num_knots(spline))
  }
...
{
  return sizeof(struct tsDeBoorNetImpl) + ts_deboornet_sof_points(net)
...
tsReal* ts_int_deboornet_access_result(const tsDeBoorNet* net)
{
  ...
  else
  {
    return ts_int_deboornet_access_points(net) +
           ...
           (ts_deboornet_len_points(net) - ts_deboornet_dimension(net));
  }
}
...
tsError ts_bspline_set_degree(tsBSpline* spline, size_t deg, tsStatus* status)
{
  ...
  {
    TS_RETURN_2(
        status, TS_DEG_GE_NCTRLP, "degree (%lu) >= num(control_points) (%lu)",
        (unsigned long)deg,
        (unsigned long)ts_bspline_num_control_points(spline))
  }
...
tsError ts_bspline_set_order(tsBSpline* spline, size_t order, tsStatus* status)
{
  ...
  {
    TS_RETURN_2(
        status, TS_DEG_GE_NCTRLP, "order (%lu) > num(control_points) (%lu)",
        (unsigned long)order,
        (unsigned long)ts_bspline_num_control_points(spline))
  }
...
{
  return ts_bspline_num_control_points(spline) * ts_bspline_dimension(spline);
}
...
tsError ts_bspline_control_points(
...
tsError ts_bspline_control_point_at(
    const tsBSpline* spline, size_t index, tsReal** ctrlp, tsStatus* status)
{
  tsReal* from;
  size_t size;
  ...
  TS_CALL(
      try, err, ts_int_bspline_access_ctrlp_at(spline, index, &from, status))
  size = ts_bspline_dimension(spline) * sizeof(tsReal);
  *ctrlp = (tsReal*)malloc(size);
  if (!*ctrlp)
  {
    TS_THROW_0(try, err, status, TS_MALLOC, "out of memory")
  }
...
tsError ts_bspline_set_control_points(
...
tsError ts_bspline_set_control_point_at(
...
tsError
ts_bspline_set_knots(tsBSpline* spline, const tsReal* knots, tsStatus* status)
{
  const size_t size = ts_bspline_sof_knots(spline);
  ...
  for (idx = 1; idx < num_knots; idx++)
  {
    ...
    {
      TS_RETURN_1(
          status, TS_KNOTS_DECR, "decreasing knot vector at index: %lu",
          (unsigned long)idx)
    }
    else
    {
    ...
    {
      TS_RETURN_3(
          status, TS_MULTIPLICITY, "mult(%f) (%lu) > order (%lu)", knot,
          (unsigned long)mult, (unsigned long)order)
    }
    lst_knot = knot;
  }
...
tsError ts_bspline_set_knot_at(
    tsBSpline* spline, size_t index, tsReal knot, tsStatus* status)
{
  ...
  TS_TRY(try, err, status)
  TS_CALL(
      try, err, ts_int_bspline_access_knot_at(spline, index, &oldKnot, status))
...
tsError
ts_deboornet_points(const tsDeBoorNet* net, tsReal** points, tsStatus* status)
{
...
tsError
ts_deboornet_result(const tsDeBoorNet* net, tsReal** result, tsStatus* status)
{
...
tsError ts_int_bspline_generate_knots(
    const tsBSpline* spline, tsBSplineType type, tsStatus* status)
{
  ...
  {
    TS_RETURN_2(
        status, TS_NUM_KNOTS, "num(knots) (%lu) %% order (%lu) != 0",
  ...
  else if (type == TS_BEZIERS)
  {
    ...
    for (i = order; i < n_knots - order; i += order)
      ts_arr_fill(knots + i, order, TS_DOMAIN_DEFAULT_MIN + (i / order) * fac);
    ts_arr_fill(knots + i, order, TS_DOMAIN_DEFAULT_MAX);
  }
  TS_RETURN_SUCCESS(status)
}
    ...
    tsBSplineType type, tsBSpline* spline, tsStatus* status)
{
  ...
  {
    TS_RETURN_2(
        status, TS_NUM_KNOTS, "unsupported number of knots: %lu > %i",
  ...
  if (degree >= num_control_points)
  {
    TS_RETURN_2(
        status, TS_DEG_GE_NCTRLP, "degree (%lu) >= num(control_points) (%lu)",
        (unsigned long)degree, (unsigned long)num_control_points)
  }
...
tsError
ts_deboornet_copy(const tsDeBoorNet* src, tsDeBoorNet* dest, tsStatus* status)
{
  size_t size;
  if (src == dest)
    TS_RETURN_SUCCESS(status)
...
tsError ts_int_thomas_algorithm(
    const tsReal* a, const tsReal* b, const tsReal* c, size_t num, size_t dim,
    tsReal* d, tsStatus* status)
{
  size_t i, j, k, l;
  tsReal m, *cc = NULL;
  tsError err;
  ...
  {
    TS_RETURN_1(
        status, TS_NUM_POINTS, "num(points) (%lu) <= 1", (unsigned long)num)
  }
  ...
  {
    TS_THROW_2(
        try, err, status, TS_NO_RESULT, "error: |%f| <= |%f|", b[0], c[0])
  }
  ...
  for (i = 1; i < num; i++)
  {
    ...
    {
      TS_THROW_3(
          try, err, status, TS_NO_RESULT, "error: |%f| <= |%f| + |%f|", b[i],
          a[i], c[i])
    }
...
tsError ts_int_relaxed_uniform_cubic_bspline(
    const tsReal* points, size_t n, size_t dim, tsBSpline* spline,
    tsStatus* status)
{
  ...
  {
    TS_RETURN_1(
        status, TS_NUM_POINTS, "num(points) (%lu) <= 1", (unsigned long)n)
  }
  ...
  TS_CALL(
      try, err,
      ts_bspline_new((n - 1) * 4, dim, order - 1, TS_BEZIERS, spline, status))
  ctrlp = ts_int_bspline_access_ctrlp(spline);
...
tsError ts_bspline_interpolate_cubic_natural(
    ...
    tsBSpline* spline, tsStatus* status)
{
  ...
  {
    TS_CALL_ROE(
        err, ts_bspline_new(4, dimension, 3, TS_CLAMPED, spline, status))
    for (i = 0; i < ts_bspline_num_control_points(spline); i++)
    {
      memcpy(
          ts_int_bspline_access_ctrlp(spline) + i * dimension, points,
          sof_ctrlp);
    }
    TS_RETURN_SUCCESS(status)
  }
  ...
  else
  {
    TS_CALL(
        try, err,
        ts_int_thomas_algorithm(a, b, c, num_int_points, dimension, d, status))
  }
  memcpy(thomas, points, sof_ctrlp);
  memmove(thomas + dimension, d, num_int_points * sof_ctrlp);
  memcpy(
      thomas + (num_int_points + 1) * dimension,
      points + (num_points - 1) * dimension, sof_ctrlp);
  TS_CALL(
      try, err,
      ts_int_relaxed_uniform_cubic_bspline(
...
tsError ts_bspline_interpolate_catmull_rom(
    const tsReal* points, size_t num_points, size_t dimension, tsReal alpha,
    const tsReal* first, const tsReal* last, tsReal epsilon, tsBSpline* spline,
    tsStatus* status)
{
  const size_t sof_real = sizeof(tsReal);
  ...
  if (num_points == 1)
  {
    TS_CALL_ROE(
        err,
        ts_bspline_new(
            num_points, dimension, num_points - 1, TS_CLAMPED, spline, status))
  ...
  for (i = 1 /* 0 (`first`) is not assigned yet */;
       i < num_points /* skip last point (inclusive end) */; i++)
  {
    ...
    if (ts_distance(p0, p1, dimension) <= eps)
    {
      ...
      if (i < num_points - 1)
      {
        memmove(p1, p1 + dimension, (num_points - (i + 1)) * sof_ctrlp);
      }
      num_points--;
      i--;
    }
  ...
  {
    memcpy(cr_ctrlp + ((num_points + 1) * dimension), last, sof_ctrlp);
  }
  else
  {
    p0 = p1 - dimension;
    for (d = 0; d < dimension; d++)
    {
      cr_ctrlp[((num_points + 1) * dimension) + d] = p1[d] + (p1[d] - p0[d]);
    }
  }
  ...
  TS_CALL(
      try, err,
      ts_bspline_new(
          (num_points - 3) * 4, dimension, 3, TS_BEZIERS, spline, status))
  bs_ctrlp = ts_int_bspline_access_ctrlp(spline);
  TS_CATCH(err)
  free(cr_ctrlp);
  TS_END_TRY_ROE(err)
...
tsError ts_int_bspline_find_knot(
    const tsBSpline* spline, tsReal knot, size_t* index, size_t* multiplicity,
    tsStatus* status)
{
  const size_t deg = ts_bspline_degree(spline);
  const size_t num_knots = ts_bspline_num_knots(spline);
  ...
  {
    TS_RETURN_2(
        status, TS_U_UNDEFINED, "knot (%f) < min(domain) (%f)", knot, min)
  }
  if (knot > max && !ts_knots_equal(knot, max))
  {
    TS_RETURN_2(
        status, TS_U_UNDEFINED, "knot (%f) > max(domain) (%f)", knot, max)
  }
    ...
    const tsBSpline* spline, tsReal u, tsDeBoorNet* net, tsStatus* status)
{
  ...
  /* 1. */
  k = s = 0;
  TS_CALL_ROE(err, ts_int_bspline_find_knot(spline, u, &k, &s, status))
  ...
  {                    /* by 3a) s <= deg (order = deg+1) */
    ...
    for (; r <= ts_deboornet_num_insertions(net); r++)
    {
      ...
      {
        ui = knots[i];
        a = (ts_deboornet_knot(net) - ui) / (knots[i + deg - r + 1] - ui);
        a_hat = 1.f - a;
    ...
    const tsBSpline* spline, tsReal u, tsDeBoorNet* net, tsStatus* status)
{
  ...
  TS_CALL(try, err, ts_int_bspline_eval_woa(spline, u, net, status))
  TS_CATCH(err)
  ts_deboornet_free(net);
  TS_END_TRY_RETURN(err)
}

tsError ts_bspline_eval_all(
    ...
    tsStatus* status)
{
  ...
  *points = (tsReal*)malloc(sof_points);
  ...
  for (i = 0; i < num; i++)
  {
    TS_CALL(try, err, ts_int_bspline_eval_woa(spline, us[i], &net, status))
    ...
    tsStatus* status)
{
  ...
  size_t i;
  if (num == 0)
    num = (ts_bspline_num_control_points(spline) - ts_bspline_degree(spline))
          * 30;
  *actual_num = num;
    ...
    tsStatus* status)
{
  ...
  {
    TS_RETURN_2(
        status, TS_INDEX_ERROR, "dimension (%lu) <= index (%lu)",
        (unsigned long)dim, (unsigned long)index)
  }
  if (max_iter == 0)
  ...
  {
    TS_THROW_1(
        try, err, status, TS_NO_RESULT, "maximum iterations (%lu) exceeded",
...
{
  *min = ts_int_bspline_access_knots(spline)[ts_bspline_degree(spline)];
  *max = ts_int_bspline_access_knots(
      spline)[ts_bspline_num_knots(spline) - ts_bspline_order(spline)];
}

tsError ts_bspline_is_closed(
...
tsError ts_int_bspline_resize(
    const tsBSpline* spline, int n, int back, tsBSpline* _resized_,
    tsStatus* status)
{
  const size_t deg = ts_bspline_degree(spline);
  const size_t dim = ts_bspline_dimension(spline);
  ...
  TS_CALL_ROE(
      err, ts_bspline_new(nnum_ctrlp, dim, deg, TS_OPENED, &tmp, status))
    ...
    tsStatus* status)
{
  ...
  TS_TRY(try, err, status)
  for (m = 1; m <= n; m++)
  { /* from 1st to n'th derivative */
    if (deg == 0)
    {
      ts_arr_fill(ctrlp, dim, 0.f);
      ts_bspline_domain(spline, &knots[0], &knots[1]);
      num_ctrlp = 1;
      num_knots = 2;
      break;
    }
    ...
    for (i = 2 * deg + 1; i < num_knots - (deg + 1); i++)
    {
      ...
      {
        TS_THROW_1(
            try, err, status, TS_UNDERIVABLE, "discontinuity at knot: %f",
            knots[i])
      }
      memmove(snd, snd + dim, (num_ctrlp - (i + 1 - deg)) * sof_ctrlp);
      memmove(&knots[i], &knots[i + 1], (num_knots - (i + 1)) * sof_real);
      num_ctrlp--;
      num_knots--;
      i += deg - 1;
    }
  ...
  TS_CALL(
      try, err, ts_bspline_new(num_ctrlp, dim, deg, TS_OPENED, &swap, status))
  memcpy(ts_int_bspline_access_ctrlp(&swap), ctrlp, num_ctrlp * sof_ctrlp);
  memcpy(ts_int_bspline_access_knots(&swap), knots, num_knots * sof_real);
...
tsError ts_int_bspline_insert_knot(
    const tsBSpline* spline, const tsDeBoorNet* deBoorNet, size_t n,
    tsBSpline* result, tsStatus* status)
{
  ...
  TS_CALL_ROE(err, ts_int_bspline_resize(spline, (int)n, 1, result, status))
  ...
  memmove(
      to, from, (num_ctrlp_result - n - (k - deg + N)) * sof_ctrlp); /* b) */
  /* copy knots */
    ...
    size_t* k, tsStatus* status)
{
  ...
  TS_CALL(
      try, err, ts_int_bspline_insert_knot(spline, &net, num, result, status))
  ts_deboornet_free(&net);
    ...
    tsStatus* status)
{
  ...
  else
  {
    TS_CALL(
        try, err,
        ts_int_bspline_insert_knot(
            spline, &net, ts_deboornet_num_insertions(&net) + 1, split,
            status))
    *k = ts_deboornet_index(&net) + ts_deboornet_num_insertions(&net) + 1;
  }
  TS_CATCH(err)
  *k = 0;
  TS_FINALLY
...
tsError ts_bspline_tension(
    const tsBSpline* spline, tsReal tension, tsBSpline* out, tsStatus* status)
{
  ...
  for (i = 0; i < N; i++)
  {
    for (d = 0; d < dim; d++)
    {
      ...
      ctrlp[i * dim + d]
          += s * (p0[d] + ((tsReal)i / (N - 1)) * (pn_1[d] - p0[d]));
    }
  }
  TS_RETURN_SUCCESS(status)
}

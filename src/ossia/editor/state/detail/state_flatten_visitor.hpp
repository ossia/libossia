...
struct vec_merger
{
  ...
  template <std::size_t N>
  ossia::state_element operator()(float orig, std::array<float, N> incoming) const
  {
  ...
  template <std::size_t N>
  ossia::state_element
  operator()(std::array<float, N>& orig, const std::array<float, N>& incoming) const
  {
    auto& existing_index = existing_dest.index;
    auto& incoming_index = incoming_dest.index;
    if(incoming_index.empty())
  ...
  template <std::size_t N>
  ossia::state_element
  operator()(std::array<float, N>& orig, const std::vector<ossia::value>& incoming) const
  {
    auto& existing_index = existing_dest.index;
    auto& incoming_index = incoming_dest.index;
    if(incoming_index.empty())
...
struct state_flatten_visitor_merger
{
  ...
  void operator()(piecewise_message& existing, const piecewise_message& incoming)
  {
    ...
    value_merger<true>::merge_list(existing.message_value, incoming.message_value);
  }

  //// Piecewise Vec Message incoming
  template <std::size_t N>
  void operator()(
      piecewise_vec_message<N>& existing, const piecewise_vec_message<N>& incoming)
  {
    ...
    else
    {
      ...
      if(!to_append_index_empty && !source_index_empty)
      {
        ...
        value_merger<true>::insert_in_list(
            pw.message_value, existing.message_value, existing.dest.index);
        value_merger<true>::insert_in_list(
            pw.message_value, std::move(incoming.message_value), incoming.dest.index);
      }
      // For these cases, we mix in the one that has the index;
      // the one without index information becomes index [0] :
      else if(!to_append_index_empty)
...
struct state_flatten_visitor
{
  ...
  template <std::size_t N>
  static ossia::net::parameter_base* param_ptr(const piecewise_vec_message<N>& m)
  {
    return &m.address.get();
  }
...
private:
  ...
  static auto find_same_param(ossia::state& st, const T& incoming)
  {
    if constexpr(AssumeSameAddresses)
    {
      ...
      return find_if(st, [&](const state_element& e) { return ossia::visit(vis, e); });
    }
    else
    {

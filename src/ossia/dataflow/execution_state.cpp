...
void execution_state::insert(ossia::net::parameter_base& param, const value_port& val)
{
  ...
  switch(val.mix_method)
  {
    case ossia::data_mix_method::mix_replace: {
      for(ossia::timed_value& v : val.get_data())
      {
        ...
        if(it != st.end())
          it->first = map_value_to_param(param, std::move(v), val);
        else
          st.emplace_back(map_value_to_param(param, std::move(v), val), idx++);
      }
      break;
    }
    case ossia::data_mix_method::mix_append: {

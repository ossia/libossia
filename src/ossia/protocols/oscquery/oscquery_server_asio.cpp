...
oscquery_server_protocol::~oscquery_server_protocol()
{
  ...
  {
    auto& dev = *m_device;
    dev.on_node_created.disconnect<&oscquery_server_protocol::on_nodeCreated>(
        this);
    dev.on_node_removing.disconnect<&oscquery_server_protocol::on_nodeRemoved>(
        this);
    dev.on_parameter_created
        .disconnect<&oscquery_server_protocol::on_parameterChanged>(this);
...
bool oscquery_server_protocol::observe(
...
bool oscquery_server_protocol::observe_quietly(
    net::parameter_base& addr, bool b)
{
...
void oscquery_server_protocol::set_device(net::device_base& dev)
{
  ...
  dev.on_parameter_created
      .connect<&oscquery_server_protocol::on_parameterChanged>(this);
  dev.on_parameter_removing
      .connect<&oscquery_server_protocol::on_parameterChanged>(this);
  dev.on_attribute_modified
...
void oscquery_server_protocol::on_osc_message(
    const oscpack::ReceivedMessage& m) try
{
  // TODO
  auto id = ossia::net::message_origin_identifier{*this, client_identifier(m_clients, {})};
...
void oscquery_server_protocol::on_connectionOpen(
    const connection_handler& hdl) try
{
    ...
    const net::node_base& n, std::string oldname) try
{
  ...
  const auto mess = ossia::oscquery::json_writer::path_renamed(old_addr, n.osc_address());
  lock_t lock(m_clientsMutex);
  for (auto& client : m_clients)
  {
    m_websocketServer->send_message(client.connection, mess);

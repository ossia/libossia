  ...
  switch(config.mode)
  {
      ...
      struct {
            ...
            -> std::unique_ptr<osc_protocol_base>
        {
          if(config.framing == conf::SIZE_PREFIX)
            return std::make_unique<osc_generic_client_protocol<client_type, tcp_size_prefix_client>>(std::move(ctx), conf);
          else
            return std::make_unique<osc_generic_client_protocol<client_type, tcp_slip_client>>(std::move(ctx), conf);
        }
...
#if defined(BOOST_ASIO_HAS_LOCAL_SOCKETS)
          if(config.framing == conf::SIZE_PREFIX)
            return std::make_unique<osc_generic_client_protocol<client_type, unix_stream_size_prefix_client>>(std::move(ctx), conf);
          else
            return std::make_unique<osc_generic_client_protocol<client_type, unix_stream_slip_client>>(std::move(ctx), conf);
#endif
          return {};
        }
        ...
        {
          if(config.framing == conf::SIZE_PREFIX)
            return std::make_unique<osc_generic_client_protocol<client_type, serial_socket<size_prefix_framing>>>(std::move(ctx), conf);
          else
            return std::make_unique<osc_generic_client_protocol<client_type, serial_socket<slip_framing>>>(std::move(ctx), conf);
        }

        auto operator()(ossia::net::ws_client_configuration&& conf) const
            -> std::unique_ptr<osc_protocol_base>
        {
          return std::make_unique<osc_generic_client_protocol<client_type, websocket_simple_client>>(std::move(ctx), conf);
        }
    ...
    case conf::HOST:
    {
      ...
      struct {
        ...
        {
          if(config.framing == conf::SIZE_PREFIX)
            return std::make_unique<osc_generic_server_protocol<client_type, tcp_size_prefix_server>>(std::move(ctx), conf);
          else
            return std::make_unique<osc_generic_server_protocol<client_type, tcp_slip_server>>(std::move(ctx), conf);
        }
...
#if defined(BOOST_ASIO_HAS_LOCAL_SOCKETS)
          if(config.framing == conf::SIZE_PREFIX)
            return std::make_unique<osc_generic_server_protocol<client_type, unix_stream_size_prefix_server>>(std::move(ctx), conf);
          else
            return std::make_unique<osc_generic_server_protocol<client_type, unix_stream_slip_server>>(std::move(ctx), conf);
#endif
          return {};
        }

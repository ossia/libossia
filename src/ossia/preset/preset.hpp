 ...
 */
OSSIA_EXPORT bool
apply_json(const std::string& json, ossia::net::node_base& node, func_t fn = nullptr);
 ...
 */
OSSIA_EXPORT void
apply_preset(const std::string& kiss, ossia::net::node_base& node, func_t fn = nullptr);

// DEPRECATED keep it for backward compatibility
OSSIA_EXPORT void apply_preset(
    ossia::net::node_base&, const presets::preset&, keep_arch_type t = keep_arch_on,
    presets::instance_functions = {}, bool allow_nonterminal = false,
    bool remove_first = true);

OSSIA_EXPORT presets::preset
make_preset(ossia::net::node_base&, preset_save_options = {});

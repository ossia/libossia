...
ossia::net::generic_device*
ZeroconfOscqueryListener::find_device(const std::string& instance)
{
  std::lock_guard<std::mutex> lock(ZeroconfOscqueryListener::m_mutex);

  auto it = ossia::find_if(
      m_devices, [&](const auto& d) { return d->get_name() == instance; });
...
{
  m_mutex.lock();
  for(auto name : m_zombie_devices)
  {
    for(auto client : ossia_pd::instance().clients.reference())
    {
      if(client->is_zeroconf() && client->m_device
         && client->m_device->get_name() == name)
      {
    ...
    ossia::remove_erase_if(
        m_devices, [&](const auto& d) { return d->get_name() == name; });
  }

...
ossia_node_t ossia_node_find(ossia_node_t node, const char* name)
{
...
void ossia_node_find_pattern(
    ossia_node_t node, const char* pattern, ossia_node_t** data, size_t* size)
{
  return safe_function(__func__, [=] {
    if(!node)
    {
...
void ossia_node_array_free(ossia_node_t* data)
{
  free(data);
}
...
float ossia_node_get_priority(ossia_node_t node, int* b)
{
...
int ossia_node_get_refresh_rate(ossia_node_t node, int* b)
{
  return safe_function(__func__, [=]() -> int {
    if(!node)
    {
...
double ossia_node_get_value_step_size(ossia_node_t node, int* b)
{
...
void ossia_node_set_instance_bounds(ossia_node_t node, int min, int max)
{
  return safe_function(__func__, [=] {
    ...
    ossia::net::set_instance_bounds(
        *convert_node(node), ossia::net::instance_bounds{min, max});
  });
}

...
public:
  ...
  virtual iterator emplace(const_iterator pos, string name) override
  {
    ...
    if (!err)
    {
      ...
      return children().insert(pos, shared_ptr<JamomaNode>(new JamomaNode(name, mDirectory, node, shared_ptr<JamomaNode>(this))));
    }
...
protected:
  ...
  void buildChildren(JamomaNode * parent)
  {
    ...
    for (childrenList.begin(); childrenList.end(); childrenList.next())
    {
      ...
      parent->children().insert(parent->children().cend(), shared_ptr<JamomaNode>(new JamomaNode(nameInstance.data(), parent->mDirectory, child, shared_ptr<JamomaNode>(parent))));
    }
  }
};

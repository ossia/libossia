...
template <>
class Device<Minuit>::Impl : public SharedImpl {
  ...
  public:
    ...
    Impl(std::string name)
    {
      ...
      // TODO : register the local application to the Minuit protocol
...
template <>
template <typename U>
Address<U> Device<Minuit>::addAddress(std::string address) const
{
  ...
  {
    // TODO : setup attribute for Minuit protocol case and depending on the U type
  }
}

template <>
template <typename U>
Address<U> Device<Minuit>::addAddress(std::string address, U min, U max) const
{
  TTAddress anAddress(address.data());
  TTSymbol  service("parameter");
  TTObject  aData;
  ...
  if (pimpl->registerDataAtAddress(anAddress, service, aData))
  {
    // TODO : setup attribute for Minuit protocol case depending on the U type
  }
}

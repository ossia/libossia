...
namespace impl
{
BasicDevice::BasicDevice(shared_ptr<Protocol> protocol, TTObject applicationManager, TTObject application, TTNodeDirectoryPtr aDirectory) :
BasicNode(aDirectory, aDirectory->getRoot()),
mProtocol(protocol),
...
string BasicDevice::getName() const
{
  TTSymbol device_name;
  mApplication.get("name", device_name);
...
Node & BasicDevice::setName(std::string name)
{
...
shared_ptr<Protocol> BasicDevice::getProtocol() const
{
  return mProtocol;
}

...
if ("$env:APPVEYOR_BUILD_TYPE" -eq "max"){
  ...
  mkdir build-32bit
  cd build-32bit
  Write-Host cmake -G ${env:CMAKE_GENERATOR_NAME} -DCMAKE_BUILD_TYPE=${env:CONFIGURATION} ${env:CMAKE_OPTION}  .. > C:\projects\libossia\config-${env:APPVEYOR_BUILD_TYPE}-32bit.log

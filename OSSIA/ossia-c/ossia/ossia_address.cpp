        ...
        ossia_value_callback_t callback)
{
    return safe_function(__func__, [=] () -> ossia_value_callback_index_t {
        if(!address) { ossia_log_error("ossia_address_add_callback: address is null"); return nullptr; }
        if(!callback) { ossia_log_error("ossia_address_add_callback: callback is null"); return nullptr; }
...
{
    return safe_function(__func__, [=] {
        if(!address) { ossia_log_error("ossia_address_remove_callback: address is null"); return; }
        if(!index) { ossia_log_error("ossia_address_remove_callback: index is null"); return; }

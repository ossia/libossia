    ...
    const char* name)
{
  return safe_function(__func__, [=] () -> ossia_device_t {
    ...
    if(it != devs.end())
    {
      ...
      if(!it->second) { ossia_log_error("ossia_device_create: it->second is null"); return nullptr; }
      return it->second;
    }
    else
    {
      if(!protocol) { ossia_log_error("ossia_device_create: protocol is null"); return nullptr; }

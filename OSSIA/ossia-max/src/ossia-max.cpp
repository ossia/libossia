...
# pragma mark Templates
    ...
    bool object_register(T* x)
    {
        ...
        int l;
        t_object* device_box = find_parent_box(&x->m_object, gensym("ossia.device"), 0, &l);
        t_object* client_box = find_parent_box(&x->m_object, gensym("ossia.client"), 0, &l);
...
/*        if (std::is_same<T, t_view>::value || std::is_same<T, t_remote>::value)
        {
            ...
            t_object* client_box = find_parent_box(object, gensym("ossia.client"), 0, &l);
           ...
           while (model_box)
            {
                ...
                vs.push_back(tmp->m_name->s_name);
                model_box = find_parent_box_alive(&tmp->m_object, gensym("ossia.model"), 1, &model_level);
            }
...
# pragma mark Utilities
    ...
    {
        t_object* parent_box = find_parent_box(object, classname, start_level, level);
        ...
        if (parent_box)
        {
            ...
            if (object)
            {
                ...
                {
                    parent_box = find_parent_box_alive(&object->m_object, classname, 1, level);

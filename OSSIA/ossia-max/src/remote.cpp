...
extern "C"
void ossia_remote_setup(void)
{
    auto& ossia_library = ossia_max::instance();
    
    // instantiate the ossia.remote class
    ossia_library.ossia_remote_class = class_new("ossia.remote",
                                                 (method)ossia_remote_new,
                                                 (method)ossia_remote_free,
                                                 (short)sizeof(t_remote),
                                                 0L, A_GIMME, 0);
    
    if (ossia_library.ossia_remote_class)
    {
        class_addmethod(ossia_library.ossia_remote_class,   (method)t_object_base::push,            "anything",     A_GIMME,        0);
        class_addmethod(ossia_library.ossia_remote_class,   (method)t_object_base::bang,            "bang",         A_NOTHING,      0);
        class_addmethod(ossia_library.ossia_remote_class,   (method)object_dump<t_remote>,          "dump",         A_NOTHING,      0);
//        class_addmethod(ossia_library.ossia_remote_class,   (method)ossia_remote_click,             "click",        A_NOTHING,     0);
        
        class_addmethod(ossia_library.ossia_remote_class, (method)ossia_remote_assist,              "assist",       A_CANT,         0);
        class_addmethod(ossia_library.ossia_remote_class, (method)ossia_remote_in_bang,             "bang",         0);
        class_addmethod(ossia_library.ossia_remote_class, (method)ossia_remote_in_int,              "int",          A_LONG,         0);
        class_addmethod(ossia_library.ossia_remote_class, (method)ossia_remote_in_float,            "float",        A_FLOAT,        0);
        class_addmethod(ossia_library.ossia_remote_class, (method)ossia_remote_in_symbol,           "symbol",       A_SYM,          0);
        class_addmethod(ossia_library.ossia_remote_class, (method)ossia_remote_in_anything,         "anything",     A_GIMME,        0);
    }
...
extern "C"
void ossia_remote_free(t_remote *x)
{
    x->m_dead = true;
    x->unregister();
...
extern "C"
void ossia_remote_assist(t_remote *x, void *b, long m, long a, char *s)
{
    if (m == ASSIST_INLET)
    {
...
/*
extern "C"
void ossia_remote_click(t_remote *x, t_floatarg xpos, t_floatarg ypos, t_floatarg shift, t_floatarg ctrl, t_floatarg alt)
{
    ...
    if (diff.count() < 200)
    {
        ...
        int l;
        t_device *device = (t_device*) find_parent(&x->m_obj, "ossia.device", 0, &l);
*/
        /*
...
void ossia_remote_in(t_remote* x, T f)
{
    ...
    {
        object_error((t_object*)x, "[ossia.remote %s] is not registered to any parameter", x->m_name->s_name);
    }
}

extern "C"
void ossia_remote_in_float(t_remote* x, double f)
{ ossia_remote_in(x, f); }

extern "C"
void ossia_remote_in_int(t_remote* x, long int f)
{ ossia_remote_in(x, (int32_t)f); }

extern "C"
void ossia_remote_in_bang(t_remote* x)
{ ossia_remote_in(x, ossia::impulse{}); }

extern "C"
void ossia_remote_in_symbol(t_remote* x, t_symbol* f)
{ ossia_remote_in(x, std::string(f->s_name)); }

extern "C"
void ossia_remote_in_char(t_remote* x, char f)
{ ossia_remote_in(x, f); }

extern "C"
void ossia_remote_in_anything(t_remote* x, t_symbol *s, long argc, t_atom *argv)
{
    if (argc == 0)
    {
    ...
    namespace max
    {
        ...
        bool t_remote :: do_registration(ossia::net::node_base* node)
        {
            ...
            if(node)
            {
                ...
                if (m_node)
                {
                    m_callbackit = m_node->get_address()->add_callback([=](const ossia::value& v){ apply_value_visitor(v); });
                    m_node->about_to_be_deleted.connect<t_remote, &t_remote::is_deleted>(this);

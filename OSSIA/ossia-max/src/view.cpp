...
extern "C"
void ossia_view_setup(void)
{
    auto& ossia_library = ossia_max::instance();
    
    // instantiate the ossia.view class
    ossia_library.ossia_view_class = class_new("ossia.view",
                                               (method)ossia_view_new,
                                               (method)ossia_view_free,
                                               (short)sizeof(t_view),
                                               0L, A_GIMME, 0);
    
    if (ossia_library.ossia_view_class)
    {
        class_addmethod(ossia_library.ossia_view_class, (method)object_dump<t_view>,    "dump",       A_NOTHING,   0);
//        class_addmethod(ossia_library.ossia_view_class, (method)ossia_view_click,       "click",      A_NOTHING,   0);
    }
...
extern "C"
void *ossia_view_new(t_symbol *name, long argc, t_atom *argv)
{
    auto& ossia_library = ossia_max::instance();
...
/*
extern "C"
void ossia_view_click(t_view *x, t_floatarg xpos, t_floatarg ypos, t_floatarg shift, t_floatarg ctrl, t_floatarg alt)
{
...
namespace ossia
{
    namespace max
    {
        ...
        bool t_view :: do_registration(ossia::net::node_base* node)
        {
            ...
            // we should unregister here because we may have add a node between the registered node and the remote
            unregister();
        ...
        {
            std::vector<box_hierachy> children_view = find_children_to_register(&x->m_object, get_patcher(&m_object), gensym("ossia.view"));

...
void ts_internal_bspline_find_u(
    const tsBSpline* bspline, const tsReal u, size_t* k, size_t* s,
    jmp_buf buf)
{
...
void ts_internal_bspline_copy(
    const tsBSpline* original, tsBSpline* copy, jmp_buf buf)
{
  const size_t dim = original->dim;
  const size_t sof_f = sizeof(tsReal);
...
void ts_internal_bspline_fill_knots(
    ...
    const tsReal max, tsBSpline* result, jmp_buf buf)
{
  const size_t n_knots = original->n_knots;
...
void ts_internal_bspline_resize(
    const tsBSpline* bspline, const int n, const int back, tsBSpline* resized,
    jmp_buf buf)
{
  const size_t deg = bspline->deg;
...
void ts_internal_bspline_evaluate(
    const tsBSpline* bspline, const tsReal u, tsDeBoorNet* deBoorNet,
    jmp_buf buf)
{
  const size_t deg = bspline->deg;
  const size_t order = bspline->order;
  ...
  {                    /* by 3a) s <= deg (order = deg+1) */
    ...
    for (; r <= deBoorNet->h; r++)
    {
      ...
      for (; i <= lst; i++)
      {
        ...
        for (d = 0; d < dim; d++)
        {
          deBoorNet->points[tidx++] = a_hat * deBoorNet->points[lidx++]
...
void ts_internal_bspline_split(
    const tsBSpline* bspline, const tsReal u, tsBSpline* split, size_t* k,
    jmp_buf buf)
{
  tsDeBoorNet net;
  tsError e;
  ...
  {
    ts_internal_bspline_insert_knot(bspline, &net, net.h + 1, split, b);
    *k = net.k + net.h + 1;
  }
  CATCH
  *k = 0;
...
void ts_internal_bspline_thomas_algorithm(
    const tsReal* points, const size_t n, const size_t dim, tsReal* output,
    jmp_buf buf)
{
...
void ts_internal_relaxed_uniform_cubic_bspline(
    const tsReal* points, const size_t n, const size_t dim, tsBSpline* bspline,
    jmp_buf buf)
{
  ...
  ts_internal_bspline_new(
      order - 1, dim, (n - 1) * 4, TS_BEZIERS, bspline, buf);
...
void ts_internal_bspline_derive(
    const tsBSpline* original, tsBSpline* derivative, jmp_buf buf)
{
  const size_t sof_f = sizeof(tsReal);
  const size_t dim = original->dim;
  const size_t deg = original->deg;
...
void ts_internal_bspline_buckle(
    const tsBSpline* bspline, const tsReal b, tsBSpline* buckled, jmp_buf buf)
{
...
void ts_internal_bspline_to_beziers(
    const tsBSpline* bspline, tsBSpline* beziers, jmp_buf buf)
{
  tsError e;
  jmp_buf b;
  const size_t deg = bspline->deg;
...
void ts_internal_bspline_set_ctrlp(
    const tsBSpline* bspline, const tsReal* ctrlp, tsBSpline* result,
    jmp_buf buf)
{
...
void ts_internal_bspline_set_knots(
    const tsBSpline* bspline, const tsReal* knots, tsBSpline* result,
    jmp_buf buf)
{
...
tsError ts_bspline_fill_knots(
    const tsBSpline* original, const tsBSplineType type, const tsReal min,
    const tsReal max, tsBSpline* result)
{
  tsError err;
  jmp_buf buf;
  TRY(buf, err)
...
tsError
ts_bspline_buckle(const tsBSpline* bspline, const tsReal b, tsBSpline* buckled)
{
  tsError err;
  jmp_buf buf;
  TRY(buf, err)

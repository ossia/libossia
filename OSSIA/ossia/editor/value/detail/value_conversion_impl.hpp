...
struct numeric_value_converter
{
  ...
  T operator()(const String& v) const {
    try {
      return boost::lexical_cast<T>(v.value);
    } catch( ... ) {
      return T{};
    }
  }
  T operator()(const Vec2f& v) const { return v.value[0]; }
  T operator()(const Vec3f& v) const { return v.value[0]; }
  T operator()(const Vec4f& v) const { return v.value[0]; }
...
struct value_converter<std::string>
{
  ...
  T operator()(const String& v) const { return v.value; }
...
T convert(const ossia::Tuple& val)
{
  ...
  T res;
  const auto N = std::min(val.value.size(), detail::array_size<T>::value);
  for(std::size_t i = 0; i < N; i++)
  {
    res[i] = convert<float>(val.value[i]);
  }
  return res;
}

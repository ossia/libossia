...
template <typename X, typename Y>
bool curve_impl<X, Y>::removePoint(X abscissa)
{
  return mPointsMap.erase(abscissa) > 0;
}

template <typename X, typename Y>
Y curve_impl<X, Y>::valueAt(X abscissa) const
{
...
template <typename X, typename Y>
X curve_impl<X, Y>::getInitialPointAbscissa() const
{
...
template <typename X, typename Y>
Y curve_impl<X, Y>::getInitialPointOrdinate() const
{
...
template <typename X, typename Y>
void curve_impl<X, Y>::setInitialPointAbscissa(X value)
{
  mInitialPointAbscissa = value;
}

template <typename X, typename Y>
void curve_impl<X, Y>::setInitialPointOrdinate(Y value)
{
...
curve_impl<X, Y>::getInitialPointAbscissaDestination() const
{
...
template <typename X, typename Y>
const ossia::Destination&
curve_impl<X, Y>::getInitialPointOrdinateDestination() const
{
  return mInitialPointOrdinateDestination;
}

template <typename X, typename Y>
void curve_impl<X, Y>::setInitialPointAbscissaDestination(
...
template <typename X, typename Y>
void curve_impl<X, Y>::setInitialPointOrdinateDestination(
    const ossia::Destination& destination)
{
...
curve_impl<X, Y>::getPointsMap() const
{
  return {mPointsMap.cbegin(), mPointsMap.cend()};
}

template <typename X, typename Y>
Y curve_impl<X, Y>::convertToTemplateTypeValue(

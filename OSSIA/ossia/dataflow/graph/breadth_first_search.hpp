...
struct two_bit_color_map_fast
{
    ...
    explicit two_bit_color_map_fast(std::size_t n, const IndexMap& index = IndexMap())
      : n(n), index(index), data((n + elements_per_char - 1) / elements_per_char)
    {
      // Fill to white
...
{
  BOOST_STATIC_CONSTANT(int, elements_per_char = two_bit_color_map_fast<IndexMap>::elements_per_char);
    ...
    two_bit_color_type value)
{
  ...
  pm.data[byte_num] =
      (unsigned char)
      ((pm.data[byte_num] & ~(3 << bit_position))
       | (value << bit_position));
}
...
{
  using namespace boost;
  //auto color = make_two_bit_color_map_fast(boost::num_vertices(g), choose_const_pmap(get_param(vis, vertex_index), g, vertex_index));

...
struct bfs_update
{
  public:
    ...
    bfs_update(Graph_T& g):
      m_color{boost::make_two_bit_color_map_fast(0, boost::get(boost::vertex_index, g.m_graph))}
    {
  ...
  private:
    ...
    using pmap_type = decltype(boost::make_two_bit_color_map_fast(0, get(boost::vertex_index, graph_t{})));
...
struct tc_update
{
  ...
  private:
    ...
    template<typename BaseGraph, typename TCGraph, typename NodeMap, typename AllNodes, typename TC, typename Devices>
    static void tc_add_addresses(BaseGraph& m_graph, TCGraph& m_sub_graph, NodeMap& m_nodes, AllNodes& m_all_nodes, TC& tc, Devices& devices)
    {
      // m_active_nodes is in topo order

...
public:
        ...
        ossia::graph_t& graph)
  {
    ...
    else
    {
      ...
      {
        graph_node* node = n.first.get();
        flow_nodes.insert({node, std::make_unique<cont_node>(flow_graph, node_exec{cur_state, *node})});
      }
    }
...
struct parallel_exec
{
    ...
    void operator()(Graph_T& g, parallel_update<Impl>& self, ossia::execution_state& e, const std::vector<ossia::graph_node*>&)
    {
      self.cur_state = &e;

...
# pragma mark Life cycle
  ...
  bool pull(Address&) const override;

  bool push(const Address&) const override;

  bool observe(Address&, bool) const override;

  bool update(Node& node) const override;

  /*! to see IPs of connected Midi devices
   \todo add options */
  std::vector<MidiInfo> scan() override;
};

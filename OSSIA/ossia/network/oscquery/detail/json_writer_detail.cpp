...
void json_writer_impl::writeKey(ossia::string_view k) const { ::write_json_key(writer, k); }
...
json_writer::string_t json_writer::query_host_info(const oscquery_server_protocol& proto)
{
...
json_writer::string_t json_writer::send_message(
    const net::address_base& addr,
    const value& val)
{
  string_t buf;
  writer_t wr(buf);

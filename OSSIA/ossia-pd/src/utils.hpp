...
void get_absolute_path_view(T* x, std::vector<std::string>& vs, Ostream& fullpath)
{
  t_view *view = nullptr;
  int view_level=0;
  int start_level = 0;
...
template<typename T, typename Ostream>
void get_absolute_path_model(T* x, std::vector<std::string>& vs, Ostream& fullpath)
{
...
template<typename T> extern void obj_dequarantining(T* x){
  ...
  if (!obj_isQuarantined<T>(x)) x->quarantine().remove_all(x);
}
